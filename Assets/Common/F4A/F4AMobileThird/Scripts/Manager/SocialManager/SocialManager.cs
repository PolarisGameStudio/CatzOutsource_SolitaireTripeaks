...
#else
            DMCStoreGameInfo store1 = GetStoreInfo();
            if (store1 != null)
            {
                return store1.UrlGameOnStore;
            }
#endif
            DMCStoreGameInfo store = StoreGameInfos.Where(st => st.StorePublishType == EStorePublishType.GooglePlayStore).FirstOrDefault();
